...
class SimulationMetrics:
    def __init__(self):
        ...
        self.agent_data = pd.DataFrame(columns=["F", "Lamb", "Simulation", "speed", "acuity", "gen", "step", "id"]).astype({
            "F" : float, "Lamb": float, "Simulation": int, "speed": float, "acuity": float, "gen": int, "step": float, "id": int})
    
    def collect_agents(self, agent_list, F, Lamb, MCsimulation, step):
        """
